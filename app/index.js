...
// console.log("what is the problemd ffefwefdffffe", db);

//   // Input fields out here
...
//   // am loading invoices from firebase out here
//   useEffect(() => {
//     const loadInvoices = async () => {
//       const snapshot = await getDocs(collection(db, "invoices"));
//       const list: Invoice[] = snapshot.docs.map((d) => ({
//         id: d.id,
//         ...d.data(),
//       })) as Invoice[];
...
//   // add and update invoice in firebase
//   const handleAddOrUpdate = async () => {
//     if (!newClient || !newInvoiceNumber || !newQuantity || !newUnitPrice) return;

//     const subtotal = parseFloat(newQuantity) * parseFloat(newUnitPrice);
...
//         await updateDoc(ref, invoiceData);

//         setInvoices(invoices.map((inv) =>
//           inv.id === editingInvoice.id ? { id: editingInvoice.id, ...invoiceData } : inv
//         ));
//       } else {
//         // ➕ ADD
//         const docRef = await addDoc(collection(db, "invoices"), invoiceData);
//         setInvoices([...invoices, { id: docRef.id, ...invoiceData }]);
//       }
//     } catch (err) {
...
//               <Text style={styles.date}>Date Issued: {item.dateIssued}</Text>
...
//               <TextInput style={styles.input} placeholder="Client Name" value={newClient} onChangeText={setNewClient} />
//               <TextInput style={styles.input} placeholder="Invoice Number" value={newInvoiceNumber} onChangeText={setNewInvoiceNumber} />
//               <TextInput style={styles.input} placeholder="Date Issued" value={newDateIssued} onChangeText={setNewDateIssued} />
//               <TextInput style={styles.input} placeholder="Due Date" value={newDueDate} onChangeText={setNewDueDate} />
//               <TextInput style={styles.input} placeholder="Description" value={newDescription} onChangeText={setNewDescription} />
//               <TextInput style={styles.input} placeholder="Quantity" keyboardType="numeric" value={newQuantity} onChangeText={setNewQuantity} />
//               <TextInput style={styles.input} placeholder="Unit Price" keyboardType="numeric" value={newUnitPrice} onChangeText={setNewUnitPrice} />
//               <TextInput style={styles.input} placeholder="Tax %" keyboardType="numeric" value={newTax} onChangeText={setNewTax} />
//               <TextInput style={[styles.input, { height: 70 }]} placeholder="Notes" multiline value={newNotes} onChangeText={setNewNotes} />

//               <TouchableOpacity style={styles.saveBtn} onPress={handleAddOrUpdate}>
//                 <Text style={styles.saveText}>{editingInvoice ? "Update Invoice" : "Save Invoice"}</Text>
//               </TouchableOpacity>
...
// NOTE: JS cannot use interface. Just keeping this as a comment.
// Invoice structure:
// {
//   id: string,
//   client: string,
